...
class ShellFormatter(Formatter):
    ...
    def short(self):
        ...
        if self.tags:
            output += "# tagged with %s" % ",".join(tags)
        ...
        for record in reversed(self.records):  # oldest first
            ...
            # TODO: the plaform information is likely to be the same for all records, or
            #       at least there will be a small number of platforms, so as for
            #       dependencies this information should be printed only at the
            #       beginning of the file.
            output += "# on %d processor(s), of which the first was a %s running %s\n" % (len(record.platforms), record.platforms[0].machine, record.platforms[0].version)
            output += "# The system had network name '%s' and IP address %s.\n\n" % (record.platforms[0].network_name, record.platforms[0].ip_addr)            
            # TODO: add info about original environment
            # TODO: add info about dependencies and versions - perhaps include an
            #       inventory of what versions are installed on the current machine
            #       if that differs - what about diffs for dependencies under version
            #       control
