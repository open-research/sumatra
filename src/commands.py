...
def run(argv):
    ...
    parser.add_option('-l', '--label', help="specify a label for the simulation. If no label is specified, the label will be based on PARAMFILE and the timestamp.")
...
def list(argv):
    ...
    description = dedent("""\
      If TAGS (optional) is specified, then only simulations with a tag in TAGS
      will be listed.""")
    parser = OptionParser(usage=usage,
                          description=description)
...
def delete(argv):
    """Delete simulation records or records with a particular tag from a simulation project."""
...
def repeat(argv):
    ...
    new_label = project.launch_simulation(original.parameters,
                                          ...
                                          reason="Repeat simulation %s" % original_label)
