...
def profile(argv):
    usage = "%(prog)s profile [profile-options] CMD [cmd-options] [cmd-args]"
    description = dedent("""
        Profile a Sumatra command. This runs the given command using
        cProfile and upon completion outputs a diagnostic message on
        the screen. It also dumps the raw profiling data into a file
        which can be analysed using Python's 'pstats' module or a
        graphical user interface like 'RunSnakeRun'. 'CMD' should be
        the exact same command that would be used without 'profile'.
        Example: "smt profile run -r 'Informative message.'
        defaults.param". Note that enabling profiling may considerably
        slow down execution.
        """)
    parser = ArgumentParser(usage=usage, description=description)
    parser.add_argument('cmds', metavar='CMD', nargs='*',
                        help="the command to be profiled. It should include "
                             "the same arguments and command line switches "
                             " that would be used without 'profile'.")
    ...
    parser.add_argument('-s', '--sorting-method', metavar='METHOD', default='cumulative',
                        help="method used to sort the profiling stats. "
                             "Can be any of the methods accepted by "
    ...
    subcmds = [cmd for cmd in argv if cmd in modes]
    i = argv.index(subcmds[0]) if subcmds != [] else len(argv)
    argv_this, argv_cmd = argv[:i], argv[i+1:]

    args = parser.parse_args(argv_this)
    if subcmds != []:
        cmd = subcmds[0]
    else:
